...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { external_place_id, tripadvisor_url_path, depth = 50, since_days = 60 } = await req.json();
    ...
    console.log(`🌐 Getting TripAdvisor Incremental Reviews for URL ${tripadvisor_url_path}, depth: ${depth} with PRIORITY=2`);
    ...
    console.log(`📋 Got ${items.length} TripAdvisor incremental reviews from DataForSEO`);
    ...
    if (rows.length) {
      ...
      console.log(`✅ Upserted ${rows.length} TripAdvisor incremental reviews`);
    }

    // Actualizar snapshot de rating si vino - USAR COLUMNA tripadvisor_ratings SEPARADA
    if (res0?.rating) {
      const tripAdvisorRatingData = {
        ...
        rating_distribution: res0.rating_distribution ?? null, // 🎯 INCLUIR distribución
        pulled_at: new Date().toISOString()
      };
      
      console.log('🏨 Updating TripAdvisor incremental ratings in separate column:', tripAdvisorRatingData);
      ...
      console.log(`📊 Updated TripAdvisor incremental rating: ${res0.rating.value}/5`);
    }
    ...
    console.error("Error in ingest-tripadvisor-reviews-incremental function:", e);
